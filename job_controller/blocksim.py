...
def blocksim_high_fidelity(
    ...
    config["sampling_timesteps"] = 5000
    ...
    config["transform_fluctuations"] = [False, True]
    
    return config_to_string(config)

# all modes
us = np.concatenate((np.linspace(0.01, 2.0, 40), np.linspace(2.0, 200, 20)))
system_sizes = [32, 64, 128, 256, 512, 1024, 2048]
for mode in [16, 17]:
    config = blocksim_high_fidelity(system_sizes, feedback_mode=mode, us=us, eq_steps=50000, sample_structure_function=True, sample_avalanche_sizes=True)
    submit_jobs(config, f"blocksim_{mode}_2048", memory="10gb", time="6:00:00", ncores=12, nodes=4, record_error=True)

...
def generate_config_very_high_fidelity(system_sizes=[128], mzr_probs=None, nruns=500, timestep_type=0, alpha=2.0, sampling_timesteps=None, equilibration_timesteps=None, sample_structure_function=True, sample_variable_mutual_information=False):
    config = {}
    config["circuit_type"] = "quantum_automaton"
...
def generate_config_very_high_fidelity_temporal(system_sizes=[128], mzr_probs=None, nruns=1, timestep_type=0, alpha=2.0, measurement_freq=1, sampling_timesteps=100):
    if mzr_probs is None:
...
if __name__ == "__main__":
    ...
    submit_jobs(config, f"qa_clean_pl_2", ncores=48, nodes=6, memory="50gb", time="48:00:00", run_local=False)
    ...
    submit_jobs(config, f"qa_hybrid", ncores=48, nodes=1, memory="50gb", time="48:00:00", run_local=False)

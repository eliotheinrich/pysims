...
def generate_config_very_high_fidelity(
    ...
    config["sample_surface_moments"] = sample_surface_moments
...
if __name__ == "__main__":
    ...
    config = generate_config_very_high_fidelity(system_sizes=system_sizes, mzr_probs=mzr_probs, timestep_type=0, nruns=100, sample_structure_function=False, sample_surface_moments=True)
    submit_jobs(config, f"rc_moments_2", ncores=8, nodes=10, memory="1gb", time="05:00:00", run_local=False)#, average_congruent_runs=False)
    #config["nruns"] = 1
    #submit_jobs(config, f"rc_moments_2", ncores=1, nodes=1, memory="1gb", time="05:00:00", run_local=True)#, average_congruent_runs=False)
